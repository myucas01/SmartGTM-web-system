          ...
          <ul>
            <li>
              <a href="javascript:void(0);"
                 ...
                 @click="goInfo"
                 style="height: auto">
                <img src="../assets/img/wdzh_tx.png"
                     alt="" />
                <div>
...
export default {
  ...
  mounted() {
    ...
    this.notReadTotal = this.readTotal
    this.$setWs.initWebSocket(
      `ws://${window.location.host}/ws-api/socket/${userID}`
    )
  },
  watch: {
    $route() {
  ...
  methods: {
    ...
    logout() {
      this.$fetch.post('/user-api/auth/logout').then(res => {
        if (res.code == 200) {
          logout()
          setTimeout(() => {
    ...
    messageInit() {
      let url = '/api/route/websocket/noread/' + this.$cookies.get('userId')
      this.$fetch.get(url).then(res => {
        if (res.code == '200') {
          this.notReadTotal = res.data
          this.$store.commit('SET_READTOTAL', res.data)
        }
      })
    }
  }
