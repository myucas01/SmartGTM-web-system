    ...
    <div class="mainBox"
      ...
      <div class="borderTitle">
        <span class="font-white">
          <i :class="[treeShow ? 'el-icon-caret-bottom' : 'el-icon-caret-right']"
             @click="changeTreeShow"></i>行政边界
        </span>
        <!--        <span class="font-white f12" style="line-height: 20px">-->
        <!--          <el-switch :width="Switchwidth" v-model="borderSwitch"></el-switch-->
        <!--          >显示边界-->
        <!--        </span>-->
        <span class="font-white f12 rightBtn"
              @click="goAdjust"
              v-auth="'orgBorder:detail:set:update'"
              v-if="isSpecific == 0">组织边界调整</span>
      </div>
      <div class="treeBox">
        <!--        <el-input-->
        <!--            v-show="treeShow"-->
      ...
      <div class="infoBox">
        <div class="lableBox"
          ...
          <span class="font-white f14"
                style="">{{ lableName }}</span>
          <el-switch v-model="otherOrgBorder"
                     @change="getOtherOrg"></el-switch>
        </div>
        <div class="lableBox">
          ...
          <el-input size="small"
                    disabled
                    class="smart_input_deep"
                    v-model="orgName"></el-input>
        </div>
        <div class="lableBox">
          <span class="lable font-white">城市/区域:</span>
          <el-cascader class="smart_cascader_deep"
                       size="small"
                       clearable
                       v-model="area"
                       ref="cascaderAddr"
                       ...
                       :props="CascaderProps"
                       @change="handleChange"
                       @visible-change="cascaderVis"></el-cascader>
        </div>
        <div class="lableBox">
      ...
      <orgstyle ref="style"
                ...
                v-show="colorShow"
                @styleBack="styleBack"
                @colorChange="colorChange"></orgstyle>
      <div class="btnBox">
        <el-button class="backBtn"
                   size="small"
                   @click="goback">取消</el-button>
        <el-button class="setBtn"
                   type="primary"
                   size="small"
                   @click="submit">设置完成</el-button>
      </div>
    </div>
    <div class="icon_open"
         v-show="!leftShow"
  ...
  methods: {
    ...
    getRegionName(lnglat) {
      ...
      geocoder.getAddress(lnglat, (status, result) => {
        if (status === 'complete' && result.regeocode) {
          this.getRegioninfo(result.regeocode.addressComponent)
        } else {
        ...
        .then((res) => {
          if (res.code == 200) {
            for (let i in this.treeChild) {
              if (this.treeChild[i].regionCode == res.data.tileId) {
    ...
    getTree() {
      ...
      if (this.orgType == 3) {
        ...
        url = '/api/boundary/region/getRegionByOrgId'
      }
      this.$fetch
        .get(url, {
          orgId: this.orgId,
        ...
        .then((res) => {
          if (res.code == 200 && res.data) {
            ...
            this.treeChild = []
            if (this.orgType == 3) {
              this.$set(this, 'treeData', res.data.orgRegionSetDTOList)
              this.$refs.tree.setCheckedKeys(res.data.tileIds)
              for (let i in this.treeData) {
                this.treeData[i].oseted = true
                this.treeData[i].istooltip = true
                for (let o in this.treeData[i].regions) {
                  this.treeData[i].regions[o].oseted = true
                  this.treeData[i].istooltip = true
                  for (let x in this.treeData[i].regions[o].regions) {
                    this.treeData[i].regions[o].regions[x].oseted = true
                    this.treeData[i].istooltip = true
                  }
                }
              }
            ...
            for (let i in this.treeChild) {
              // if (this.orgType == '3') {
              //   if (!this.treeChild[i].oSeted) {
              //     //没被其他组织占用
              //     this.drawOrganizeBorder(this.treeChild[i])
              //   }
              // }
    ...
    drawOrganizeColor(
      ...
      option.color = this.orgColor ? this.orgColor : '#FF9D9D'
      option.fillColorOpacity = this.orgOpacity ? this.orgOpacity : 50
      let obj = {
        data: [option],
        zIndex: zIndex,
        bubble: false,
    ...
    colorChange(color, opacityVal) {
      ...
      if (this.orgType == '0') {
        ...
        list = data.filter((item) => item.regionLevel == '5')
      }
      this.orgColor = color
      for (let i in list) {
        list[i].color = color
