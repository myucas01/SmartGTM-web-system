...
import {textEllipsis} from './filters'
...
let MapCtrl = {
  ...
  drawBorders: (map_obj, options) => {
    ...
    for (let i = 0; i < length; i++) {
      (function (t) {
        ...
        // 获取GeoJSON数据并转化为对象
        gdGeom = typeof info.gdBoundarJson === 'string' ? JSON.parse (info.gdBoundarJson) : info.gdBoundarJson;
        // console.log('拿到的gdGeom数据', option.strokeColor)
        // 实例化一个高德GeoJSON对象
  ...
  drawNames: (map_obj, options) => {
    ...
    const defaultOption = {
      ...
      prefisso: '',
      markCode: '',
      needTextEllipsis: false,
      TextEllipsisLength: 10
    };
    const option = {
      ...defaultOption,
      ...options,
      ...
      if (options.data[i].gdLng) {
        (function (t) {
          ...
          const map_text_obj = new AMap.Text ({
            text: option.needTextEllipsis? textEllipsis(name, option.TextEllipsisLength) : name ,
            title: name,
            anchor: 'center',
            position: textCenter,
            style: options.style,
          ...
          info.DrawType = 'name';
          eventMap.id = info[options.markCode];
