	...
	<div class="map-search">
		...
		<div class="s-content">
			<el-tabs v-model="typeData" @tab-click="handleClick">
				<el-tab-pane label="地图搜索" name="mapTab" v-if="searchType.indexOf('map') >= 0">
					<div class="tip2" v-if="cityObj.citycode">搜索范围: {{ cityObj.province }}</div>
					...
					<div>
						<el-input v-model="addressName" placeholder="搜索道路,POI  <<enter搜索>>" suffix-icon="el-icon-search" clearable @keyup.enter.native="searchAddress" @clear="clearAddress" class="smart_input_deep"></el-input>
					</div>
					<div class="s-address">
						<el-collapse v-if="mapData.length > 0">
							<el-collapse-item v-for="(item, index) in mapData" :title="`${index + 1}-${item.name}`" :key="index">
								<p>名称：{{ item.name }}</p>
								<p>地址：{{ item.pname }} - {{ item.cityname }} - {{ item.adname }} - {{ item.address }}</p>
							</el-collapse-item>
						</el-collapse>
					</div>
					<div class="s-bottom" v-if="mapData.length > 0">
						<el-pagination small layout="prev, next" @current-change='handleCurrentChange'></el-pagination>
					</div>
				</el-tab-pane>
				<el-tab-pane label="门店搜索" name="storeTab" v-if="searchType.indexOf('org') >= 0" v-model="adminStoreData">
					...
					<div>
						<el-input v-show="adminStoreData == 'administrative'" v-model="storeParams.storeNameCode" placeholder="请输入 门店编号/门店名称" suffix-icon="el-icon-search" clearable class="smart_input_deep" @keyup.enter.native="searchOrgAllStore" @clear="clearOrgAllStore"></el-input>
					</div>
					<div class="s-address">
						<el-collapse accordion v-if="storeData.length > 0" @change="handleChange" v-model="storeParams.storeCodeId">
							<el-collapse-item v-for="(item, index) in storeData" :title="`${index + 1}-${item.storeName.substring(0, 15)}`" :key="index" :name="`${item.storeId}`">
					...
					<!-- <div>
						...
						suffix-icon="el-icon-search" clearable class="smart_input_deep" @keyup.enter.native="searchAdminStore" @clear="clearStore"></el-input>
						<el-input v-show="adminStoreData == 'organizational'" v-model="orgParams.storeNameCode" placeholder="请输入门店名称"
						suffix-icon="el-icon-search" clearable class="smart_input_deep" @keyup.enter.native="searchOrgStore" @clear="clearStore"></el-input>
					</div> -->
					<!-- <el-tabs v-model="adminStoreData">
						<el-tab-pane label="行政边界" name="administrative">
							<div class="s-address_admin">
								<el-tree ref="adminTreeData" :props="defaultPropsAdmin" lazy :load="loadNodeAdmin" class="smart_tree_deep"
								node-key="id" highlight-current @node-click="storeNodeClick"></el-tree>
							</div>
						</el-tab-pane>
						<el-tab-pane label="组织边界" name="organizational">
							<div class="s-address_admin">
								<el-tree ref="orgTreeData" :props="defaultPropsOrg" lazy :load="loadNodeOrg" class="smart_tree_deep" node-key="id"
								highlight-current @node-click="orgNodeClick"></el-tree>
							</div>
						</el-tab-pane>
						<el-tab-pane label="商圈边界" name="business">
							<el-tree :data="treeData" :props="defaultProps" class="smart_tree_deep"></el-tree>
						</el-tab-pane>
					</el-tabs> -->
				</el-tab-pane>
			</el-tabs>
		...
		methods: {
			...
			async searchOrgAllStore() {
				...
				if (_this.organizationId && _this.storeParams.storeNameCode) {
					...
					if(this.searchType.indexOf('opportunity_store') >= 0){//店群画像 搜索门店专用接口
						params.seedOrBusiness = 'business'
						params.storesId= this.$route.query.storesId
					}else if(this.searchType.indexOf('seed_store') >= 0){
						params.seedOrBusiness = 'seed'
						params.storesId= this.$route.query.storesId
					}
					_this.$fetch.get(url, params).then((res) => {
						if (res.code == 200) {
				...
				for (let item of data) {
					if (item.storeLng && item.storeLat) {
						...
						let marker = new AMap.Marker({
							...
							offset: new AMap.Pixel(3, -5) //(-10, -30)
						})
						if (lng == 1) {
							_this.map_obj.setFitView(marker)
						}
