    ...
    <div class="nav-list">
      <ul class="outer-ul">
        <li v-for="(item, i) in nav" :key="i">
          ...
          <ul class="sub-ul" v-show="item.open">
            <li v-for="(subItem, j) in item.list" :key="j">
              <h4>
                <a
                  :class="subItem.cur ? 'cur' : ''"
                  href="javascript:void(0);"
                ...
                >
                  ...
                  <i
                    v-if="subItem.url == '' || subItem.url == null"
                    class="el-icon-arrow-down"
                  ></i>
                </a>
              </h4>
              <ul class="sub-ul" v-show="subItem.open">
                <li v-for="(minItem, k) in subItem.list" :key="k">
                  <h5>
                    <a
                      :class="minItem.cur ? 'cur' : ''"
                      href="javascript:void(0);"
                      @click.stop="
                        toChannelC(
                          item.menuName,
              ...
              if (secItem.url) {
                switch (secItem.url) {
                  ...
                  case '/index/page/menu':
                    ...
                    break
                  case '/business-expansion/dashboard-grafana':
                    window.open(
                      'http://demo.parramountain.com/d/XtK_XzQMk/ocm_gaia_store?orgId=2&from=1616010401175&to=1616032001176&var-province=All&var-type=All'
                    )
                    break
                  case '/kmrs/index':
                    window.open('http://192.168.0.101:96/index', '_blank')
                    break
                  case '/kmrs/export/merge-data-export':
                    window.open(
                      'http://192.168.0.101:96/export/merge-data-export',
                      '_blank'
                    )
                    break
                  case '/kmrs/word/word-list':
                    window.open(
                      'http://192.168.0.101:96/word/word-list',
                      '_blank'
                    )
                    break
                  case '/kmrs/record/index':
                    window.open(
                      'http://192.168.0.101:96/record/index',
                      '_blank'
                    )
                    break
                  default:
