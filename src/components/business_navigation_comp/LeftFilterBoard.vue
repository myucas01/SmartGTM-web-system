...
<template>
	<div class="leftFilerBoard" id="left_filter_board">
		...
		<section class="breadcrumb">
			...
			<span class="text">{{ '网点分布 / ' + breadName }}</span>
		</section>
		<!--顶部选项卡按钮-->
		...
		<section class="content-wrap">
			...
			<div class="content">
				<!--问题门店过滤器-->
				<store-filter
					v-show="activeTitle === '门店筛选'"
					@generateEcharts="generateEcharts"
					v-bind="$attrs"
					v-on="$listeners"
				/>
				<!-- <business-store-filter v-show="activeTitle === '门店筛选' && type === 2" @change="storeChanged" /> -->
				<!--门店属性过滤器-->
				<attribute-filter
				...
				/>
				<!--店群画像Echarts-->
				<store-charts v-show="activeTitle === '店群画像'" v-bind="$attrs" v-on="$listeners"/>
			</div>
		</section>
	</div>
</template>
...
<script>
	import StoreFilter from './panelGroup/storeFilter'
	// import BusinessStoreFilter from "./panelGroup/businessStoreFilter";
	import AttributeFilter from './panelGroup/attributeFilter'
	import StoreCharts from './panelGroup/storeCharts'
	...
	export default {
		...
		created() {},
		mounted() {
			// 获取路由参数panelType：sale || problem，根据参数动态修改左侧控制面板显示tab
			// 如果是售点盘点模块，则只有2个选项卡
			if (this.$route.path === '/business-navigate/sale') {
				...
				// 如果是问题门店模块，则只有3个选项卡
				this.tabsData = [
					{ id: 100, text: '门店筛选', icon: 'el-icon-sort' },
					{ id: 111, text: '属性筛选', icon: 'el-icon-goods' },
					{ id: 222, text: '店群画像', icon: 'el-icon-s-data' }
				]
				// 默认选中门店筛选
