<template>
	<div class="map-top-operate">
		...
		<div class="icon-change" v-show="iconChange" @mouseover="functionListShow()" @mouseout="functionListNone()">
			...
			<ul class="level-lip">
				<li v-for="(item, index) in functionListData" :key="index" @mouseover="showLevel(item.type)" @click="clickLevel(item.type)" v-show="item.show">
...
<script>
	...
	export default {
		...
		beforeMount() {},
		destroyed() {
			this.$store.dispatch('map/setMapOperation', 'search, mapType, ranging')
		},
		mounted() {
		...
		methods: {
			changeFunction() {
				...
				for (var i  = 0; i < this.functionListData.length; i ++) {
					this.$set(this.functionListData[i], 'show', this.mapOperation.indexOf(this.functionListData[i].type) > -1)
				}
			},
			functionListShow() {
				let that = this
