<template>
  <div id="map-body">
    ...
    <MapSearch v-if="mapSearch" :AMap="AMap" :map_obj="map_obj"></MapSearch>
    <Organization v-if="mapOrganization"></Organization>
		<Administration v-if="mapAdministration"></Administration>
    <slot name="business-preview-cut"></slot>
  </div>
</template>
<script>
/* eslint-disable */
      ...
      for (var i = 0; i < data.length; i += 1) {
        markers.push(
          new AMap.Marker({
            ...
            content:
              '<div style="background-color: hsla(250, 100%, 50%, 0.7); height: 24px; width: 24px; border: 1px solid hsl(180, 100%, 40%); border-radius: 12px; box-shadow: hsl(180, 100%, 50%) 0px 0px 1px;" onclick="console.log(123);"><img style="width:24px; height: 24px;" src="' +
              data[i].properties.typeIcon +
              '"/></div>',
